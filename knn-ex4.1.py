...
df = pd.read_csv('Dados_Normalizados_11Features/TrainingData_11F_Norm.txt', delimiter='\t')
dd = pd.read_csv('Dados_Normalizados_11Features/TestingData_11F_Norm.txt', delimiter='\t')
...
def euclidianDistance(firstInstance, secondInstance, actualInstanceIndex):
    ...
    for key in firstInstance.keys():
        if(actualInstanceIndex == 3 and key ==  "citric.acid" and key != "ID"):
            sum += pow(0.3 - secondInstance[key], 2)
        elif key != "ID":
